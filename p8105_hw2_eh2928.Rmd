...
filter(transit_df, ada == TRUE) %>% 
  distinct(line, station_name) %>% 
    count()
```
```{r what proportion of station entrances/exits without vending allow entrance}
filter(transit_df, vending == FALSE) %>% 
    filter(entry == TRUE) %>% 
      count()
```
...
Now, reformat data so that route number and route name are distinct variables. How many distinct stations serve the A train? Of the stations that serve the A train, how many are ADA compliant?
